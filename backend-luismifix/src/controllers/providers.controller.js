...
export const createProvider = async (req, res) => {
    try {
        ...
        if (nameProvider.length < 5) return res.status(400).json({ message: 'El nombre del proveedor debe de tener al menos 5 caracteres' });
        if (noteProvider.length < 10) return res.status(400).json({ message: 'El nombre del proveedor debe de tener al menos 5 caracteres' });
        if (nameProvider.length > 50) return res.status(400).json({ message: 'El nombre del proveedor no puede tener más de 50 caracteres' });
        if (noteProvider.length > 255) return res.status(400).json({ message: 'La nota del proveedor no puede tener más de 350 caracteres' });
